...
// Lifted from apollo's 30 to 150 so long-flight intercepts (slow lobby shots
// taken with few ships) are not rejected as "unreachable" at training-label
// extraction time. 150 ≈ max sim time the engine's predict_fleet_collision uses.
pub const AIM_HORIZON: i64 = 150;
