...
/// Per-source defense summary, memoized across every (target × offset)
/// evaluated in one recompute pass (one [`run_strategy`] greedy iteration, or
/// one [`select_best_uncached`] call). Both components are independent of the
/// target and the launch offset:
///   * `not_doomed` ([`baseline_owns`]) depends only on the turn's world state.
///   * `holds` / `half_pressure` ([`neighbor_holds_under_worst_case`])
///     additionally depend on `plan`, which is fixed within one recompute pass.
/// `holds` / `half_pressure` are only meaningful when `not_doomed` is true —
/// the worst-case sim is skipped otherwise, mirroring the original control flow
/// in [`collect_source_candidates`].
#[derive(Clone, Copy)]
struct SourceDefense {
    not_doomed: bool,
    holds: bool,
    half_pressure: i64,
}

type SourceDefenseCache = HashMap<i64, SourceDefense>;

/// Cached [`baseline_owns`] + [`neighbor_holds_under_worst_case`] for `src`. A
/// source that neighbors many targets is otherwise re-checked once per
/// (target × offset), each `neighbor_holds_under_worst_case` running a full
/// `simulate_planet_timeline`; this collapses that to one computation per
/// source per recompute pass.
fn source_defense(
    world: &WorldState,
    model: &HellburnerModel,
    src: &Planet,
    plan: &PlanState,
    cache: &mut SourceDefenseCache,
) -> SourceDefense {
    if let Some(&d) = cache.get(&src.id) {
        return d;
    }
    let not_doomed = baseline_owns(world, src.id);
    let (holds, half_pressure) = if not_doomed {
...
fn evaluate_frontline_strategy(
    ...
    plan: &PlanState,
    offset: i64,
    defense_cache: &mut SourceDefenseCache,
    ctx: &TargetContext,
...
fn select_best_uncached(
    ...
    for &tid in candidate_ids {
        let Some((score, win)) = evaluate_target(
            world,
            model,
            plan,
            world.planet(tid),
        ...
        let primary = strategy.key(score, production, ships_total) * prior_multiplier(world, tid);
...
/// Target-predictor prior config. `floor` drops candidate targets whose model
/// prob is below the threshold before any evaluation; `alpha` scales the
/// selection key by `(1 + alpha * prior)` so high-prior targets win ties /
/// nudge ranking among similarly-scored candidates. Read once from env vars on
/// first access.
struct PriorConfig {
    floor: f64,
    alpha: f64,
...
fn prior_config() -> &'static PriorConfig {
    static CFG: OnceLock<PriorConfig> = OnceLock::new();
    CFG.get_or_init(|| {
        let floor = std::env::var("APOLLO_TF_PRIOR_FLOOR")
            .ok()
            .and_then(|s| s.parse::<f64>().ok())
            .unwrap_or(0.05);
        let alpha = std::env::var("APOLLO_TF_PRIOR_ALPHA")
            .ok()
            .and_then(|s| s.parse::<f64>().ok())
            .unwrap_or(2.0);
        PriorConfig { floor, alpha }
    })
}

/// Returns the per-target prior in `[0, 1]` if priors are set on the world.
/// `None` means "no priors threaded in" — callers should treat that as
/// pass-through (apollo's stock behavior).
#[inline]
fn target_prior(world: &WorldState, target_id: i64) -> Option<f64> {
    world
        .target_priors
        .and_then(|m| m.get(&target_id).copied())
}

/// Multiplier applied to the selection key. Returns 1.0 (identity) when the
/// world has no priors. Otherwise `1 + alpha * prior` (prior clamped to
/// `[0, 1]` so a stray out-of-range value can't flip signs).
#[inline]
fn prior_multiplier(world: &WorldState, target_id: i64) -> f64 {
    match target_prior(world, target_id) {
        None => 1.0,
        Some(p) => 1.0 + prior_config().alpha * p.clamp(0.0, 1.0),
    }
}

/// True iff the world has priors AND this target's prior falls below the
/// configured floor. Used to skip evaluation on targets the model is confident
/// won't be picked. When no priors are set, every target passes.
#[inline]
fn prior_filtered_out(world: &WorldState, target_id: i64) -> bool {
    match target_prior(world, target_id) {
        None => false,
        Some(p) => p < prior_config().floor,
    }
}
...
fn run_strategy(
    ...
    //
    // When target-predictor priors are threaded in (set only by
    // `apollo_bridge::apollo_candidates` for our root player), drop candidates
    // below the model floor. Opponent and rollout worlds have
    // `target_priors = None` so this is a no-op for them — apollo's stock
    ...
    let candidate_ids: Vec<i64> = world
        ...
        .filter(|p| !prior_filtered_out(world, p.id))
        .map(|p| p.id)
        .collect();
    ...
    // of T's inbound sources (availability + worst-case defense). Committing
    // target `C` from sources `S` mutates `spent[s]` for `s ∈ S` and
    // `planned[C]`, so the exactly-affected set is
    ...
    // commitment.
    for _ in 0..256 {
        // Recompute dirty targets against the current plan. A fresh defense
        // cache is consistent with the current plan and is populated lazily for
        // just the sources the recomputed targets touch.
        if !dirty.is_empty() {
            let mut defense_cache: SourceDefenseCache = HashMap::default();
            for &tid in &dirty {
        ...
        // order so ties resolve exactly as the uncached path did. When priors
        // are threaded in, multiply the strategy key by `(1 + alpha·prior)`
        // so the model nudges ranking without overriding apollo's scoring.
