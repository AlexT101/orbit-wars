...
pub struct WorldState<'a> {
    ...
    /// transformer. Set only by `apollo_bridge::apollo_candidates` (the root
    /// candidate generator for our player) — rollout-internal opponent worlds
    /// and test worlds leave it `None`, so apollo behaves identically to
    /// upstream when no priors are threaded in.
