...
fn candidates_from_ledger(
    ...
    // Only at the MCTS root do we honor the per-turn priors stashed by main.rs;
    // applying them during expansion/rollouts injects a turn-0 distribution into
    // simulated future states the model never trained on. When `use_priors` is
    // false, build the world directly without consulting the thread-local.
    if use_priors {
        crate::target_priors::with_priors_for(player, |priors| {
            let mut world = WorldState::from_simulator_with_ledger(player as i64, sim, ledger, cache);
            world.target_priors = priors;
...
pub fn apollo_candidates_pair(
    ...
    let ledger = ArrivalLedger::build(&sim, HORIZON, cache);
    (
        candidates_from_ledger(&sim, &ledger, me, cache, is_root),
        candidates_from_ledger(&sim, &ledger, opp, cache, is_root),
    )
}
...
pub fn apollo_candidates(state: &GameState, player: i32, cache: &EntityCache, is_root: bool) -> Vec<Vec<Action>> {
    let planets: Vec<APlanet> = state.planets.iter().map(to_apollo_planet_current).collect();
    let initial_planets: Vec<APlanet> =
        state.planets.iter().map(to_apollo_planet_initial).collect();
    let fleets: Vec<AFleet> = state.fleets.iter().map(to_apollo_fleet).collect();
    ...
    // Only honor the per-turn priors at the MCTS root. During expansion /
    // rollouts the simulated future state diverges from the real observation
    // the predictor was trained on, so the prior becomes stale and biases the
    // rollout policy back toward an out-of-distribution decision. (Always-on
    // priors lost 1W/11L vs prometheus_v1 in initial testing.)
    crate::target_priors::with_priors_for(player, |priors| {
        // Build inside the closure so `world` borrows priors with a lifetime
        // that ends before the closure returns. The conversion vectors above
        // are moved in here (they are owned, no lifetime conflict).
        ...
        world.target_priors = if is_root { priors } else { None };
