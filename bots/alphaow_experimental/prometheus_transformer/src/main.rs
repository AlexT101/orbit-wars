...
fn main() -> io::Result<()> {
    ...
    let budget_ms: u64 = std::env::var("ALPHAOW_BUDGET_MS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(500);
    let mut dump: Option<File> = std::env::var("ALPHAOW_DUMP_FEATURES_PATH")
        .ok()
        .and_then(|p| File::create(p).ok());
    if dump.is_some() {
        eprintln!("[alphaow] dumping features (input_dim={}) to ALPHAOW_DUMP_FEATURES_PATH", INPUT_DIM);
    }
    ...
    loop {
        ...
        // Per-turn target-predictor prior stash. Clear first so a missing
        // `target_priors` field on this turn can't reuse a prior turn's stash.
        // The Python wrapper inserts `{"target_priors": {pid: prob, ...}}`
        // when it wants apollo's root candidate generator to honor a prior.
        alphaow_bot::target_priors::reset();
        if let Some(priors_obj) = v.get("target_priors").and_then(|x| x.as_object()) {
            let mut map: rustc_hash::FxHashMap<i64, f64> = Default::default();
            for (k, val) in priors_obj.iter() {
                if let (Ok(pid), Some(prob)) = (k.parse::<i64>(), val.as_f64()) {
                    map.insert(pid, prob);
                }
            }
            if !map.is_empty() {
                alphaow_bot::target_priors::set(state.player, map);
            }
        }
        ...
        alphaow_bot::focused_plan::reset_cache();
        ...
        if debug {
            ...
            writeln!(
                err,
                "[alphaow p{}] step={} planets={}(m)/{}(n)/{}(e) ships={} fleets={} moves={}",
