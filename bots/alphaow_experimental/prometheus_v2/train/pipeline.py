...
def train_xgb(
    ...
    fine_tune_rounds: int | None,
) -> Path:
    import xgboost as xgb
    from model_dashboard import render_xgb_dashboard, training_curve_from_xgb

    if model_out.exists() and not force and fine_tune_from is None:
    ...
    resume_booster = None
    resume_model_arg = None
    resume_base_rounds = 0
    if fine_tune_from is not None:
        fine_tune_from = fine_tune_from.expanduser().resolve()
        if not fine_tune_from.exists():
            raise FileNotFoundError(f"--fine-tune-from model not found: {fine_tune_from}")
        resume_booster = xgb.Booster()
        resume_booster.load_model(str(fine_tune_from))
        n_features = int(resume_booster.num_features())
        if n_features != X.shape[1]:
            raise ValueError(
                f"--fine-tune-from has {n_features} features, but this dataset has {X.shape[1]} features"
            )
        resume_base_rounds = int(resume_booster.num_boosted_rounds())
        resume_model_arg = str(fine_tune_from)
        print(f"fine-tuning from existing booster: {fine_tune_from} ({resume_base_rounds} trees)")
    ...
    pred_transform = regression_prob if objective == "regression" else None

    def score_raw_predictions(pred_raw: np.ndarray) -> tuple[np.ndarray, float, float | None]:
        if objective == "regression":
            pred_prob = regression_prob(pred_raw)
            acc = float(((pred_raw > 0.0) == (y_va_raw > 0.0)).mean())
            rmse = float(np.sqrt(np.mean((pred_raw - y_va_raw) ** 2)))
            return pred_prob, acc, rmse
        pred_prob = pred_raw
        acc = float(((pred_prob > 0.5) == (yb_va > 0.5)).mean())
        return pred_prob, acc, None

    def best_booster_view(bst):
        best_iteration = getattr(bst, "best_iteration", None)
        if best_iteration is None:
            return bst
        end = int(best_iteration) + 1
        if 0 < end < int(bst.num_boosted_rounds()):
            return bst[:end]
        return bst

    def curve_best_iteration(bst, evals_result: dict) -> int | None:
        best_iteration = getattr(bst, "best_iteration", None)
        if best_iteration is None:
            return None
        rel = int(best_iteration) - resume_base_rounds
        n = max(len(v) for split in evals_result.values() for v in split.values()) if evals_result else 0
        return rel if 0 <= rel < n else None

    def build_extra_baselines() -> list[dict]:
        extra_baselines = []
    ...
    if resume_booster is not None:
        pred_raw = resume_booster.predict(dva)
        pred_prob, acc, rmse = score_raw_predictions(pred_raw)
        base_cfg = {
            "name": f"{model_prefix}_{model_tag}_existing",
            "max_depth": None,
            "learning_rate": 0.0,
            "n_est": 0,
        }
        rmse_suffix = "" if rmse is None else f" rmse={rmse:.4f}"
        print(f"{base_cfg['name']}: existing acc={100*acc:.2f}%{rmse_suffix}")
        best = (
            (acc, -rmse if rmse is not None else 0.0),
            base_cfg,
            resume_booster,
            acc,
            rmse,
            ...
            pred_transform,
            [],
            build_extra_baselines(),
            base_cfg["name"],
        )

    for cfg in configs:
        cfg = dict(cfg)
        if resume_model_arg is not None:
            cfg["name"] = f"{cfg['name']}_resume"
            cfg["n_est"] = fine_tune_rounds or min(240, cfg["n_est"])
            cfg["learning_rate"] = min(float(cfg["learning_rate"]), 0.03)
        params = dict(
        ...
        bst = xgb.train(
            ...
            xgb_model=resume_model_arg,
        )
        bst_eval = best_booster_view(bst)
        pred_raw = bst_eval.predict(dva)
        pred_prob, acc, rmse = score_raw_predictions(pred_raw)
        if objective == "regression":
            print(
                f"{cfg['name']}: win_acc={100*acc:.2f}% rmse={rmse:.4f} "
                f"iter={bst.best_iteration} trees={bst_eval.num_boosted_rounds()} sec={time.time()-t0:.1f}"
            )
        else:
            print(
                f"{cfg['name']}: acc={100*acc:.2f}% "
                f"iter={bst.best_iteration} trees={bst_eval.num_boosted_rounds()} sec={time.time()-t0:.1f}"
            )
        extra_baselines = build_extra_baselines()
        training_curve = training_curve_from_xgb(evals_result, curve_best_iteration(bst, evals_result))
    ...
    if dashboard_html is not None:
        render_xgb_dashboard(
            ...
            y_val=y[val_mask],
            pred_prob=best[5],
            pred_transform=best[6],
            feature_name_list=feature_name_list,
            phase_frac=observed_game_progress(meta[val_mask]),
            top_n=24,
            permutation=True,
            permutation_rows=20000,
            training_curve=best[7],
            extra_baselines=best[8],
            html_out=dashboard_html,
        )
        ...
        json.dumps(
            {
                ...
                "fine_tune_from": str(fine_tune_from) if fine_tune_from is not None else None,
...
def main() -> None:
    ...
    p.add_argument("--fine-tune-from", type=Path, default=None, help="continue training from an existing XGBoost JSON model")
    p.add_argument("--fine-tune-rounds", type=int, default=None, help="boosting rounds to append in --fine-tune-from mode; default caps each config at 240")
