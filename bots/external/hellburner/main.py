...
class Hellburner:
    ...
    def build_proximity_graph(self) -> None:
        ...
        # self.outbound_edges[p] = [(dst, travel)] â€” keyed by source, complement of the inbound-keyed inbound_edges.
    ...
    def evaluate_frontline_strategy(self, target: HPlanet) -> tuple[FleetOrders, list[Intercept], bool]:
        ...
        # Scale enemy ships at the target down to 50% â€” only half may actually threaten us.
        ...
        for neighbor, _ in possible_origins:
            ...
            # Only relevant when neighbor survives in the baseline â€” if it's already doomed, send freely.
            ...
            if not_doomed:
                ...
                if exposed_owner != self.player:
                    ...
                    if target.production <= neighbor.production:
                        continue
                    # Knowingly sacrificing neighbor â€” send all ships.
                else:
            ...
            if trial_end_owner == self.player:
                ...
                if not_doomed:
                    ...
                    if trimmed < ships_to_send:
                        ...
                        # If t_travel is not finite, trimmed fleet can't reach target â€” keep original.
