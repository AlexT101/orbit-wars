...
import random # just in case, for future ideas
import time
import functools
from collections import defaultdict, namedtuple # useful data structures
...
CENTER_Y = 50.0
SUN_R = 10.0
MAX_SPEED = 6.0 # max speed of our ships
SUN_SAFETY = 1.5 # how close can we get to the sun?
ROTATION_LIMIT = 50.0
TOTAL_STEPS = 500
SIM_HORIZON = 50 # shorter lookahead for simpler logic
ROUTE_SEARCH_HORIZON = 30 # shorter search for routes
HORIZON = SIM_HORIZON # use the simpler horizon
...
Planet = namedtuple("Planet",["id","owner","x","y","radius","ships","production"]) # planet data
...
def dist(ax,ay,bx,by): # calculate distance between two points
    return math.hypot(ax-bx, ay-by)  # oops, extra spaces
...
def predict_target_position(target, turns, world, comets, comet_ids):
    if target.id in comet_ids:
        return predict_comet_position(target.id, comets, turns)
    return world.predict_planet_position(target.id, turns)
...
def search_safe_intercept(src, target, ships, world, comets, comet_ids):
    best = None
    best_score = None
    ...
    for candidate_turns in range(1, max_turns + 1):
        pos = predict_target_position(target, candidate_turns, world, comets, comet_ids)
        if pos is None:
            continue
        ...
        actual_pos = predict_target_position(target, actual_turns, world, comets, comet_ids)
...
def aim_with_prediction(src, target, ships, world, comets, comet_ids):
    est = estimate_arrival(src.x, src.y, src.radius, target.x, target.y, target.radius, ships)
    if est is None:
        if not target_can_move(target, world.initial_by_id, comet_ids):
            return None
        return search_safe_intercept(src, target, ships, world, comets, comet_ids)

    tx, ty = target.x, target.y
    for _ in range(2): # fewer iterations, less precise but faster
        _, turns = est
        pos = predict_target_position(target, turns, world, comets, comet_ids)
        if pos is None:
            return None
        ntx, nty = pos
...
class WorldModel:
    ...
    # @functools.lru_cache(maxsize=None) # caching is for pros, let's keep it simple
    def predict_planet_position(self, planet_id, turns):
        ...
        new_ang = cur_ang + self.ang_vel * turns
    ...
    def plan_shot(self, src_id, target_id, ships):
        ...
        result = aim_with_prediction(src, target, ships, self, self.comets, self.comet_ids)
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    if target.id in world.comet_ids:
        life = world.comet_life(target.id)
        # Longer-lived comets warrant less discount — scale COMET_VALUE_MULT up
        # from 0.65 (life≈0) toward 0.84 (life≥20 turns) so short-lived comets
        # are deprioritised while healthy comets are treated more like planets.
        life_frac = min(1.0, life / 20.0)
        value *= COMET_VALUE_MULT * (1.0 + 0.50 * life_frac) # Increased scaling for longer-lived comets

    # Exposed-enemy bonus: enemy planets with fewer ships than ~5 turns of
    # production have recently dumped a large fleet and are vulnerable.
    if target.owner not in (-1, world.player):
        expected_min = target.production * 5
        if target.ships < expected_min:
            exposure_ratio = 1.0 - target.ships / max(1, expected_min)
            value *= 1.0 +0.15 * exposure_ratio  # up to +15 % for near-empty
...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    ...
    margin = 2 # a small buffer is always good
    if target.owner == -1:
        margin += target.production // 2 # neutral planets need less ships to take over
    else:
        margin += target.production # enemy planets need more ships
    if world.is_static(target.id):
        margin += 1 # static targets are easier to hit
    if world.is_four_player:
        margin += 1 # more players, more chaos, more ships needed
    if target.id in world.comet_ids:
        margin = max(0, margin - 1) # comets are tricky, send less if its a comet
