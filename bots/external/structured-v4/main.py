...
FFA_LEADER_TARGET_MULT = 1.24
FFA_NONLEADER_HOSTILE_MULT = 0.90
FFA_LONG_NONLEADER_WAR_MULT = 0.82
FFA_WEAK_PICKOFF_MULT = 1.08
FFA_WEAK_EARLY_SUPPRESSION_MULT = 0.88
FFA_SAFE_NEUTRAL_EMPIRE_MULT = 1.16
...
def simulate_planet_timeline(planet, arrivals, player, horizon):
    """Forward-simulate one planet's local future.

    This is deliberately planet-local: all in-flight fleets have already been
    projected to an arrival ledger. The function keeps dense owner/ship arrays
    because downstream ownership and rescue queries are O(1) lookups, but uses
    list-indexed event buckets rather than hash lookups in the hot loop.

    Combat model: same-turn arrivals are aggregated by owner, the two strongest
    incoming groups cancel, and the survivor resolves against the incumbent
    garrison. This mirrors the Planet Wars linear attrition family while handling
    FFA same-turn collisions.
    """
    horizon = max(0, int(math.ceil(horizon)))
    events = normalize_arrivals(arrivals, horizon)

    by_turn = [[] for _ in range(horizon + 1)]
    has_enemy_arrival = False
    for item in events:
        by_turn[item[0]].append(item)
        if planet.owner == player and item[1] not in (-1, player):
            has_enemy_arrival = True
...
class WorldModel:
    def __init__(
        ...
        self.enemy_owner_strength = {
            owner: strength
            for owner, strength in self.owner_strength.items()
            if owner not in (-1, player)
        }
        self.leader_owner = (
            max(self.enemy_owner_strength, key=self.enemy_owner_strength.get)
            if self.enemy_owner_strength
            else None
        )
        self.weakest_enemy_owner = (
            min(self.enemy_owner_strength, key=self.enemy_owner_strength.get)
            if self.enemy_owner_strength
            else None
        )
        self.max_enemy_strength = max(self.enemy_owner_strength.values(), default=0)
        ...
        self.keep_needed_map = {
            planet.id: self.base_timeline[planet.id]["keep_needed"]
            for planet in planets
        }
        self.min_owned_map = {
        ...
        self.first_enemy_map = {
            planet.id: self.base_timeline[planet.id]["first_enemy"]
            for planet in planets
        }
        self.fall_turn_map = {
    ...
    def ffa_owner_pressure_multiplier(self, owner, arrival_turns):
        """Target-owner prior for four-player free-for-all games.

        The goal is not pacifism; it is avoiding kingmaking. Hostile attacks are
        best when they reduce the current leader, finish a weak player quickly,
        or exploit a crash. Long attacks on middle-ranked players are discounted
        because both sides lose tempo while a third player compounds production.
        """
        if not self.is_four_player or owner in (-1, self.player):
            return 1.0
        strength = self.owner_strength.get(owner, 0)
        max_enemy = max(1, self.max_enemy_strength)
        if owner == self.leader_owner:
            return FFA_LEADER_TARGET_MULT
        if strength <= 0.55 * max_enemy:
            return FFA_WEAK_PICKOFF_MULT if self.remaining_steps < 130 else FFA_WEAK_EARLY_SUPPRESSION_MULT
        if arrival_turns > 12:
            return FFA_LONG_NONLEADER_WAR_MULT
        return FFA_NONLEADER_HOSTILE_MULT
    ...
    def reaction_times(self, target_id):
        ...
        for planet in self.my_planets:
            seeded = self.best_probe_aim(
                planet.id, target.id, max(1, int(planet.ships))
            )
            if seeded is None:
                continue
            _, aim = seeded
    ...
    def min_ships_to_own_by(
        ...
        def owns_at(ships):
            owner_after, _ = self.projected_state(
                ...
                planned_commitments=planned_commitments,
                extra_arrivals=normalized_extra
                + ((arrival_turn, attacker_owner, int(ships)),),
            )
...
def build_policy_state(world, deadline=None):
    ...
    for target in world.planets:
        ...
        if target.owner == world.player:
            continue
        my_sources = nearest_sources_to_target(
    ...
    for planet in world.my_planets:
        ...
        if world.remaining_steps < 200:
            decay = world.remaining_steps / 200.0
            proactive_keep = int(proactive_keep * decay)
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    if target.id in world.comet_ids:
        ...
        if target.production > 0:
            ...
            recoup_time = cost / target.production
            if arrival_turns + recoup_time > life:
                return -1.0

    value = target.production * turns_profit
    value += (
        policy["indirect_wealth_map"][target.id] * turns_profit * INDIRECT_VALUE_SCALE
    )
    ...
    if modes["is_ffa"]:
        if target.owner == -1 and is_safe_neutral(target, policy):
            value *= FFA_SAFE_NEUTRAL_EMPIRE_MULT
        elif target.owner not in (-1, world.player):
            value *= world.ffa_owner_pressure_multiplier(target.owner, arrival_turns)
...
def reinforce_value(target, hold_until, world, policy):
    ...
    value += (
        policy["indirect_wealth_map"][target.id]
        * saved_turns
...
def preferred_send(
    ...
    margin_decay = world.remaining_steps / TOTAL_STEPS
    margin = int(margin * margin_decay)
...
def build_rescue_missions(world, policy, planned_commitments, modes):
    ...
    for target in world.my_planets:
        ...
        for src in world.my_planets:
            ...
            value = (
                target.production * saved_turns
                + max(0, target.ships) * DEFENSE_SHIP_VALUE
            )
            if (
                world.enemy_planets
...
def plan_moves(world, deadline=None):
    ...
    for src in world.my_planets:
        ...
        for target in world.planets:
            ...
            if global_needed <= 0:
                continue
            if opening_filter(
                target, rough_turns, global_needed, src_available, world, policy
            ):
                continue
            ...
            for i in range(len(top_options)):
                for j in range(i + 1, len(top_options)):
                    for k in range(j + 1, len(top_options)):
                        ...
                        value = target_value(
                            target, joint_turn, "swarm", world, modes, policy
                        )
                        if value <= 0:
                            continue
                ...
                for j in range(i + 1, len(top_options)):
                    for k in range(j + 1, len(top_options)):
                        for l in range(k + 1, len(top_options)):
                            ...
                            if len({option.src_id for option in quartet}) < 4:
                                continue
                            quart_tol = swarm_eta_tolerance(
                                tuple(quartet), target, world
                            )
                            turns = [option.turns for option in quartet]
                            if max(turns) - min(turns) > quart_tol:
                                continue

                            joint_turn = max(turns)
                            total_cap = sum(option.send_cap for option in quartet)
                            need = world.min_ships_to_own_at(
                                target.id,
                                joint_turn,
                                world.player,
    ...
    for mission in missions:
        ...
        if mission.kind in (
            ...
            if mission.kind == "reinforce":
                ...
                plan = settle_plan(
                    ...
                    policy,
                    mission="snipe",
                    eval_turn_fn=lambda turns, enemy_eta=option.anchor_turn: max(
                        turns, enemy_eta
                    ),
                    anchor_turn=option.anchor_turn,
                )
        ...
        owner_after, _ = world.projected_state(
            ...
            planned_commitments=planned_commitments,
            extra_arrivals=[
                (turns, world.player, send) for _, _, turns, send in reaimed
            ],
        )
        ...
        for src in world.my_planets:
            ...
            for target in world.planets:
                ...
                if rough_needed <= 0 or rough_needed > src_left:
                    continue
                if opening_filter(
                    target, est_turns, rough_needed, src_left, world, policy
                ):
                    continue
    ...
    if world.step % 18 == 0 and world.enemy_planets and allow_optional_phase():
        others = {
            oid: s
            for oid, s in world.owner_strength.items()
            if oid != world.player and oid != -1
        }
        if others:
            strongest_id = max(others, key=others.get)
            targets = [p for p in world.planets if p.owner == strongest_id]
            if targets:
                for src in sorted(
                    world.my_planets,
                    key=lambda p: source_attack_left(p.id),
                    reverse=True,
                ):
                    if source_attack_left(src.id) < 20:
                        continue  # Higher surplus required
                    target = min(targets, key=lambda p: planet_distance(src, p))
                    aim = world.plan_shot(src.id, target.id, 1)
                    if aim:
                        angle, _, _, _ = aim
                        append_move(src.id, angle, 1)
                        break

    if world.is_total_war and world.enemy_planets and allow_optional_phase():
        weakest_enemy = min(
            world.enemy_planets, key=lambda e: world.owner_strength.get(e.owner, 0)
        )
        for src in world.my_planets:
            if expired():
                return finalize_moves()
