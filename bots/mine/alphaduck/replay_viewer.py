...
def precompute(game, ck, model):
    ...
    for t, state in enumerate(parsed):
        ...
        for player in (0, 1):
            try:
                fake = {p["id"]: 0 for p in state["planets"]}
                feats, globals_, pids = bd.extract_per_player(state, player, fake)
            except Exception:
            ...
            with torch.no_grad():
                logits = model(torch.from_numpy(pf_n),
                               torch.from_numpy(gl_n),
                               torch.from_numpy(mk)).numpy()[0]
    ...
    <div class="panel">
      <div class="stats">
        <div class="col p0">
          ...
          <div class="v" style="color:#fde047">eval: <span id="p0-eval">-</span></div>
        </div>
        <div class="col p1">
          <h4>P1 (red)</h4>
          ...
          <div class="v" style="color:#fde047">eval: <span id="p1-eval">-</span></div>
        </div>
      </div>
    </div>
    <div class="panel">
...
function renderDetail(s) {
  ...
  title.textContent = `planet ${selectedPid} (${planet.owner === 0 ? "P0" : planet.owner === 1 ? "P1" : "neut"}, ships=${planet.ships}, prod=${planet.prod})`;
...
def main():
    ...
    ap.add_argument("--ckpt", type=Path,
                    default=ROOT / "bots" / "mine" / "target_predictor" / "train" / "weights" / "transformer_pair.pt")
