...
For each replay:
  - For each tick, for each player slot, take the observation as the
    bot saw it.
  - Pipe it through `alphaow-bot` running at ALPHAOW_BUDGET_MS=1 with
    ALPHAOW_DUMP_FEATURES_PATH set so the bot dumps the 46-d
...
def spawn_bot(dump_path: Path) -> subprocess.Popen:
    ...
    env["OW_VALUE_NET"] = "0"
    env["ALPHAOW_BUDGET_MS"] = "1"  # near-zero search; we only want features
    env["ALPHAOW_DUMP_FEATURES_PATH"] = str(dump_path.resolve())
...
def main():
    ...
    scratch = Path(tempfile.mkdtemp(prefix="alphaow_replay_"))
