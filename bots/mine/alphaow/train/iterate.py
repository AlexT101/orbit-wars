"""Self-play improvement loop for alphaow.

Round r starts with weights from round r-1 (or no weights at round 0).
For each round:
  1. Collect K games of (alphaow_r-1 vs alphaow_r-1) self-play, plus optional cross-opponent games.
  2. Train a fresh MLP on the accumulated data (all rounds so far).
  3. Evaluate alphaow_r vs heuristic / apollo_fast.
...
def collect_round(r: int, prev_weights: Path | None, games_self: int, games_cross: int, budget_ms: int, base_seed: int):
    ...
    if games_cross > 0:
        args_cross = [
            ...
            "--budget-ms", str(budget_ms),
            "--pairings", "alphaow:apollo_fast:1,alphaow:heuristic:1",
