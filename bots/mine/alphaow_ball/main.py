"""Self-play wrapper: alphaow BALLISTIC rollout (OW_ROLLOUT_REACTIVE=2).

Pins OW_ROLLOUT_REACTIVE=2 so the rollout replans both players for the first 2
ticks then goes ballistic (no new launches; fleets keep moving, combat and
production still resolve). This is ~2.2x cheaper per rollout, buying ~1.57x more
MCTS iterations. Pins its own subprocess env so it can run head-to-head against
alphaow_base in the same Python process without racing on global os.environ.
Apollo rollout is left at its default (ON).
"""
...
def _ensure():
    ...
    if _PROC is not None and _PROC.poll() is None:
        return _PROC
    env = dict(os.environ)
    env["OW_ROLLOUT_REACTIVE"] = _REACTIVE
