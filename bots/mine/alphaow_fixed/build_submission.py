"""Build a Kaggle-ready submission bundle for alphaow_fixed.

alphaow_fixed is the alphaow Rust bot paired with the *fixed-extrapolation*
XGB model (xgb_top10_d6_fixed.json, retrained 2026-05-30). It is a
subprocess-style bot: a Python wrapper (main.py) spawns the `alphaow-bot`
...
The only differences from alphaow_newest's bundle are the weights file
(the *_fixed.json variant) and that main.py sets ALPHAOW_EXTRAP_FIX=1 so
...
Usage:
    python bots/mine/alphaow_fixed/build_submission.py

Output: bots/mine/alphaow_fixed/submission.tar.gz
"""
...
WEIGHTS = ALPHAOW / "train" / "weights" / "xgb_top10_d6_fixed.json"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with tarfile.open(bundle, "w:gz") as tar:
            for name in ("main.py", "alphaow-bot", WEIGHTS_NAME):
    ...
    print(f"  kaggle competitions submit orbit-wars -f {bundle} -m 'alphaow_fixed'")
    return 0
