"""alphaow_fixed — the alphaow Rust bot paired with the *fixed-extrapolation*
XGB value net, retrained 2026-05-30:
  ...
  1. **Default model**: weights/xgb_top10_d6_fixed.json — XGBoost gbtree
     ...
     paired with ALPHAOW_EXTRAP_FIX=1 (set below) so the deployed bot's
  ...
  2. **XGBoost-in-Rust** value net (src/xgb.rs) — pure-Rust gbtree
     ...
     when ALPHAOW_VALUE_NET_PATH points at a .json file.
...
This single file serves BOTH layouts — `_locate()` auto-detects which:
  ...
  * **dev**: this wrapper sits in `alphaow_fixed/` and shares the sibling
    `alphaow/` build tree (binary at `../alphaow/target/release/alphaow-bot`,
    weights at `../alphaow/train/weights/xgb_top10_d6_fixed.json`). Builds the
    binary on demand if missing.

  * **Kaggle submission**: `main.py`, `alphaow-bot`, and
...
def _locate():
    ...
    # Dev layout: this wrapper lives in alphaow_fixed/ and shares the sibling
    # alphaow/ build tree. _wrapper_dir() would return alphaow_fixed/, which
    # has no target/ or train/ — so resolve the sibling crate.
    base = wd or "/Users/derekwang/Documents/GitHub/orbit-wars/bots/mine/alphaow_fixed"
