...
BIN = ALPHAOW_DIR / "target" / "release" / "summary_parity"
...
def py_summary_from_obs(obs):
    ...
    proc = subprocess.run(
        [str(ALPHAOW_DIR / "target" / "release" / "summary_parity")],
...
def main():
    ...
    for seed in seeds:
        ...
        env = {
            "PATH": "/usr/bin:/bin",
            "ALPHAOW_DUMP_FEATURES_PATH": str(dump_path),
            "ALPHAOW_BUDGET_MS": "5",
        }
        p = subprocess.Popen(
            [str(ALPHAOW_DIR / "target" / "release" / "alphaow-bot")],
