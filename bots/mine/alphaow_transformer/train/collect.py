"""Self-play / cross-bot data collector for alphaow's value net.

Drives the Rust engine, runs N matches between configurable bot pairings,
and for each side that runs as `alphaow`, captures the per-turn feature
vectors that the bot dumps via `ALPHAOW_DUMP_FEATURES_PATH`. Each turn's
...
# parents: train -> alphaow -> mine -> bots -> repo
ROOT = Path(__file__).resolve().parents[4]
BOTS_DIR = ROOT / "bots"
ALPHAOW_DIR = ROOT / "bots" / "mine" / "alphaow"
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))
BIN_PATH = ALPHAOW_DIR / "target" / "release" / "alphaow-bot"
...
class AlphaowDaemon:
    """Direct subprocess driver for alphaow-bot with per-process env control.

    Bypasses the Python wrapper, which captures os.environ at spawn time
    and so leaks env between concurrent alphaow players in the same
    Python process.
    """

    def __init__(self, dump_path: Path | None, budget_ms: int, weights_path: Path | None, value_net_off: bool):
        env = dict(os.environ)
        env.pop("ALPHAOW_DUMP_FEATURES_PATH", None)
        env.pop("ALPHAOW_VALUE_NET_PATH", None)
        env.pop("OW_VALUE_NET", None)
        if dump_path is not None:
            env["ALPHAOW_DUMP_FEATURES_PATH"] = str(Path(dump_path).resolve())
        env["ALPHAOW_BUDGET_MS"] = str(budget_ms)
        if weights_path is not None:
            env["ALPHAOW_VALUE_NET_PATH"] = str(Path(weights_path).resolve())
        if value_net_off:
            env["OW_VALUE_NET"] = "0"
...
def load_other_agent(name: str):
    """Load a non-alphaow bot via its main.py."""
...
def main():
    ...
    p.add_argument("--budget-ms", type=int, default=200, help="alphaow MCTS budget per turn")
    p.add_argument(
        "--pairings",
        default="alphaow:alphaow:1.0",
        help="Comma-separated bot1:bot2:weight; one is sampled per game. Use alphaow on at least one side to collect data.",
    )
    p.add_argument("--seed", type=int, default=0, help="base seed (game_i uses seed+i)")
    p.add_argument("--weights", default=None, help="ALPHAOW_VALUE_NET_PATH for collection rollouts (None = heuristic)")
    args = p.parse_args()
    ...
    scratch = Path(tempfile.mkdtemp(prefix="alphaow_collect_"))
