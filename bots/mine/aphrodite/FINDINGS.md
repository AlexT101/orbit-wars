...
- aphrodite = duck (Decoupled-UCT) + learned value-net leaf evaluator.
...
- vs heuristic at 500ms budget: ~80% win rate (baseline aphrodite
...
- `APHRODITE_VALUE_NET_PATH` — load weights from this AOWV file.
...
- `OW_K_ROOT`, `OW_K_NON_ROOT` — override MCTS candidate counts
  (defaults 5 / 4).
- `APHRODITE_BUDGET_MS` — override MCTS wall budget.
- `APHRODITE_DUMP_FEATURES_PATH` — dump per-turn 2728-d features to a
...
3. **apollo_fast-style training data**: most of our data is aphrodite
   vs (heuristic | apollo_fast). Mirror games with apollo_fast vs
   itself (with aphrodite as silent observer that just extracts
