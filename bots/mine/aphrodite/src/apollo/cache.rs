...
use crate::apollo::engine::{fleet_speed, CometGroup, Planet};

/// Aim solver result tuple: `(angle, turns, target_x, target_y,
/// fractional_flight_time)`. See [`crate::apollo::aim::AimResult`].
...
/// per-turn comet gate ([`crate::apollo::aim::comet_blocks_path`]) handles them.
...
pub enum InvariantVerdict {
    ...
    /// gates the carried base): solve normally with [`crate::apollo::aim::aim_with_prediction`].
    SingleSolve,
    /// Unknown (cache miss) and potentially cacheable: solve the comet-free base
    /// with [`crate::apollo::aim::aim_ignoring_comets`] and feed it to
...
pub struct Entity {
    ...
    /// it lets [`crate::apollo::aim::comet_blocks_path`] reject, in O(1) with no
    ...
    /// contiguous band, which [`crate::apollo::aim::blocked_on_path`] binary-searches
...
pub struct EntityCache {
    ...
    /// ([`crate::apollo::aim::blocked_on_path`] / `cone_clear_impossible`) iterate
    ...
    /// ([`crate::apollo::aim::comet_blocks_path`]) iterates only comets instead of
...
impl EntityCache {
    ...
    /// [`crate::apollo::aim::aim_ignoring_comets`] `comet_free` base (the shot clear
    ...
    /// ray, identical to the one [`crate::apollo::aim::shot_blocked_exact`] sweeps.
