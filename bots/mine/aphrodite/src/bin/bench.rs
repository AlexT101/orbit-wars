...
use aphrodite::{parse_state, ow2_plan, policy};
...
fn main() {
    ...
    for _ in 0..(n/10) {
        ...
        for _ in 0..8 {
            ...
            aphrodite::sim::apply_launches(&mut s, &a);
            aphrodite::sim::apply_launches(&mut s, &b);
            aphrodite::sim::tick(&mut s);
            sink += s.fleets.len() as i64;
        }
    }
    ...
    for _ in 0..n {
        let mut s = state.clone();
        for _ in 0..15 { aphrodite::sim::tick(&mut s); }
