...
fn detect_kind(input_dim: usize) -> Option<InputKind> {
    if input_dim == INPUT_DIM {
        ...
        eprintln!(
            "[aphrodite] unknown input_dim={} (expected {} full / {} summary / {} summary_v2 / {} summary_v3)",
            input_dim,
            INPUT_DIM,
...
fn parse_weights(bytes: &[u8]) -> Option<MlpWeights> {
    ...
    match version {
        ...
        v => {
            eprintln!("[aphrodite] unsupported weights version {}", v);
            None
        }
    }
}
...
fn parse_v2(bytes: &[u8], input_dim: usize, kind: InputKind) -> Option<MlpWeights> {
    ...
    if *out_dims.last()? != 1 {
        eprintln!("[aphrodite] v2 weights: final layer out_dim must be 1");
        return None;
    }
    ...
    match &w {
        Some(mw) => {
            let arch: Vec<String> = std::iter::once(mw.input_dim)
                ...
                .collect();
            eprintln!(
                "[aphrodite] loaded value net (kind={:?}, arch={}) from {}",
                mw.kind,
                arch.join("->"),
                path
            );
        }
        None => eprintln!("[aphrodite] failed to parse value net weights at {}", path),
    }
    w.map(Model::Mlp)
}
