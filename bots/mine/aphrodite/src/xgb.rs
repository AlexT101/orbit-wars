...
pub fn load(bytes: &[u8]) -> Option<XgbModel> {
    let dump: XgbDump = match serde_json::from_slice(bytes) {
        Ok(d) => d,
        Err(e) => {
            eprintln!("[aphrodite] xgb JSON parse error: {}", e);
            return None;
        }
    };
    ...
    for t in &dump.learner.gradient_booster.model.trees {
        ...
        {
            eprintln!("[aphrodite] xgb tree has inconsistent per-node array lengths");
            return None;
        }
        ...
        for i in 0..n {
            ...
            if lc < 0 {
                ...
                if fi < 0 || (fi as usize) >= num_feature {
                    eprintln!("[aphrodite] xgb tree feat_idx {} out of range", fi);
                    return None;
                }
