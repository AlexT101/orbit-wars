...
def agent(obs):
    ...
    # markers describing candidate moves). Older builds of apollo_native do
    # not expose `take_debug`, so a stale `.extracted/` .so would otherwise
    # crash here with AttributeError on every turn — guard via getattr.
