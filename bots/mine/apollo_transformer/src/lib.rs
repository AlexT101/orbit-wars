...
impl Bot {
    ...
    #[pyo3(signature = (obs, target_priors=None))]
    fn compute_moves_with_search(
        &mut self,
        obs: &Bound<'_, PyDict>,
        target_priors: Option<&Bound<'_, PyDict>>,
    ) -> PyResult<Vec<(i64, f64, i64)>> {
        let obs = Observation::from_dict(obs)?;
        self.refresh_cache(&obs);
        let shot_l1 = crate::world::ShotL1::default();

        // Parse the optional target-predictor priors: {planet_id: prob}.
        // Threaded only through the top-level candidate WorldState used for
        // *our* turn-0 planning; the rollout's opponent/replan WorldStates
        // continue to use `None`, so apollo's stock behavior is preserved
        // everywhere except where the model can actually inform our move.
        let priors_map: Option<rustc_hash::FxHashMap<i64, f64>> = match target_priors {
            Some(d) => Some(parse_target_priors(d)?),
            None => None,
        };
        ...
        let (candidates, initial_ledger) = {
            ...
            world.remaining_overage_time = obs.remaining_overage_time;
            world.shot_l1 = Some(&shot_l1);
            world.target_priors = priors_map.as_ref();
...
/// Parse a `{planet_id: prob}` Python dict into a Rust `HashMap<i64, f64>`.
/// Accepts integer or float-valued ids (the same robustness `extract_i64` adds
/// for observation parsing). Probabilities outside `[0, 1]` are kept verbatim
/// here and clamped by `prior_multiplier` at use; the dict is small (~30
/// entries max) so a one-pass copy is fine.
fn parse_target_priors(
    d: &Bound<'_, PyDict>,
) -> PyResult<rustc_hash::FxHashMap<i64, f64>> {
    let mut out: rustc_hash::FxHashMap<i64, f64> =
        rustc_hash::FxHashMap::with_capacity_and_hasher(d.len(), Default::default());
    for (k, v) in d.iter() {
        let pid = extract_i64(&k)?;
        let prob: f64 = v.extract()?;
        out.insert(pid, prob);
    }
    Ok(out)
}
...
#[pymodule]
fn apollo_transformer_native(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<Bot>()?;
