...
/// world has no priors. Otherwise `1 + alpha * prior`, where the prior is
/// clamped into `[0, 1]` so a stray out-of-range value can't flip signs.
#[inline]
...
fn run_strategy(
    ...
    //
    // When the target-predictor priors are threaded in (set only on the
    // top-level WorldState for our turn-0 planning), drop candidates below the
    // model floor. Opponent and rollout worlds have `target_priors = None` so
    // this is a no-op for them — apollo's stock behavior is preserved.
    ...
    for _ in 0..256 {
        ...
        // are threaded in, the strategy key is multiplied by `(1 + alpha·prior)`
        // so the model's per-target probability boosts apollo's own scoring
