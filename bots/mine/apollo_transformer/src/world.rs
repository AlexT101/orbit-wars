...
pub struct WorldState<'a> {
    ...
    /// Per-planet prior P(my side targets planet) from the target-predictor
    /// transformer. Set only for the top-level WorldState that drives our
    /// own turn-0 planning (`Bot::compute_moves_with_search`); rollout-internal
    /// opponent worlds and test worlds leave it `None`, so apollo behaves
    /// identically to upstream when no priors are threaded in.
    pub target_priors: Option<&'a HashMap<i64, f64>>,
}
