"""beam — same machinery as alphaow_newest (apollo candidates + XGB value
net + sim::tick) but the planner is a beam search instead of DUCT MCTS.

Tunables (env vars, both default reasonable):
  * OW_BEAM_WIDTH  (default 16) — beam width
  * OW_BEAM_DEPTH  (default 6)  — max depth

Original alphaow_newest docs below for reference.
...
def _ensure():
    ...
    for k in (
        ...
        # Beam-specific overrides also stripped so this wrapper is a stable
        # baseline. Set them in your shell to experiment.
        # "OW_BEAM_WIDTH", "OW_BEAM_DEPTH",  # leave passthrough for now
    ):
        env.pop(k, None)
    # Beam-search planner instead of DUCT. Reuses the same apollo
    # candidate generator and XGB value net (so the only difference vs
    # alphaow_newest is "tree → beam"). OW_BEAM_WIDTH and OW_BEAM_DEPTH
    # are honored by the planner.
    env["OW_PLANNER"] = "beam"
    # Rollouts are irrelevant for beam (it doesn't simulate to leaves; it
    # evaluates with the value net at every node) but keep the flags set
    # so any inadvertent fallback isn't expensive.
    env["OW_ROLLOUT"] = "none"
