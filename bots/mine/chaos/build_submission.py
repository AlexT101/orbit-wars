...
def _stage(td: Path, cdylibs: dict[str, Path]) -> list[str]:
    files: list[tuple[Path, str]] = [
        (HERE / "main.py", "main.py"),
        (APHRODITE / "main.py", "aphrodite_wrapper.py"),
        (BIN_OUT, "aphrodite"),
...
# Steady-state turns must stay under the 1s act timeout.
for turn in range(3):
    ...
    if turn > 0:
        assert dt < 950, f"steady-state turn took {dt:.0f}ms — over Kaggle's 1s act timeout"
