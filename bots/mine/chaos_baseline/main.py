...
if _BUNDLE:
    ...
    ROOT = _repo_root()
    # chaos_baseline: A/B clone that drives the *baseline* aphrodite binary
    # (flat bundle under bots/mine/aphrodite_baseline) instead of the dev build,
    # so two chaos instances can be compared with different aphrodite engines.
    # Everything else (IL transformer, checkpoints, torch) is identical to chaos.
    _APH_WRAPPER = ROOT / "bots" / "mine" / "aphrodite_baseline" / "main.py"
