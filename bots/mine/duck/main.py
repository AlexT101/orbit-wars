"""Kaggle entry point for duck. Spawns the Rust binary once and pipes
one JSON observation per turn.

Binary path resolution: $DUCK_BOT_BIN, then <this dir>/target/release/duck-bot.
...
def _here():
    try:
        ...
        env = os.environ.get("DUCK_BOT_DIR")
        if env:
            return env
        for cand in (os.getcwd(), os.path.join(os.getcwd(), "duck")):
...
def _binary_path():
    env = os.environ.get("DUCK_BOT_BIN")
    if env and os.path.isfile(env):
        return env
    return os.path.join(_here(), "target/release/duck-bot")
