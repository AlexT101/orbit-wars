...
use duck_bot::{parse_state, ow2_plan, policy};
...
fn main() {
    ...
    for _ in 0..(n/10) {
        ...
        for _ in 0..8 {
            ...
            duck_bot::sim::apply_launches(&mut s, &a);
            duck_bot::sim::apply_launches(&mut s, &b);
            duck_bot::sim::tick(&mut s, &mut r);
            sink += s.fleets.len() as i64;
        }
    }
    ...
    for _ in 0..n {
        let mut s = state.clone();
        let mut r = policy::XorRng(s.step as u64 | 1);
        for _ in 0..15 { duck_bot::sim::tick(&mut s, &mut r); }
