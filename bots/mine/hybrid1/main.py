...
CHECKPOINT = Path(os.environ.get("HYBRID1_IL_CHECKPOINT", HERE / "osteo_il_latest.pt")).expanduser()
...
torch.set_num_threads(int(os.environ.get("HYBRID1_TORCH_THREADS", "1")))
...
def _pump_stderr(pipe) -> None:
    try:
        for line in iter(pipe.readline, b""):
            if os.environ.get("HYBRID1_DEBUG"):
...
def _aphrodite_env() -> dict:
    env = dict(os.environ)
    env.setdefault("APHRODITE_BUDGET_MS", env.get("HYBRID1_APHRODITE_BUDGET_MS", "500"))
...
def _start_aphrodite():
    binary = Path(os.environ.get("HYBRID1_APHRODITE_BIN", HERE / "aphrodite")).expanduser()
...
class ILAgent:
    def __init__(self) -> None:
        self.device = torch.device(os.environ.get("HYBRID1_IL_DEVICE", "cpu"))
        deterministic = os.environ.get("HYBRID1_IL_DETERMINISTIC", os.environ.get("OSTEO_IL_DETERMINISTIC", ""))
...
def _use_aphrodite(payload: dict) -> bool:
    return len(_alive_players(payload)) > 2 or _IL is None
...
def agent(obs, config=None):
    ...
    if os.environ.get("HYBRID1_DEBUG"):
        sys.stderr.write(f"[hybrid1] step={payload['step']} alive={len(_alive_players(payload))} choice={choice}\n")
        sys.stderr.flush()
    if choice == "il" and _IL is not None:
        try:
            ...
            if os.environ.get("HYBRID1_DEBUG"):
                sys.stderr.write(f"[hybrid1] IL error: {exc!r}\n")
                sys.stderr.flush()
            return []
