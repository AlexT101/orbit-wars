//! Pathing primitives.
//!
//! `dir_to_hit(source, target, num_ships, ...)` returns `(angle, time)` where
//! `time` follows the production-count convention: a `time` of `k` means the
//! fleet collides on the (k)-th game turn from now, so the target produces `k`
//! times during the flight (one production per fleet movement turn).
//!
//! Algorithm:
//!   - For each candidate arrival turn t = 1..MAX_TIME, project target to its
//!     position at t and compute the arc of launch angles whose source-circle
//!     of radius `speed*t` intersects the target circle.
//!   - Subtract obstacles per-t: for each other planet/comet, for each
//!     intermediate turn k = 1..=t, if the obstacle at turn k lies within
//!     ~one fleet step of the fleet's distance ring at k, subtract that arc.
//!     This covers stationary obstacles (static every turn) and moving ones
//!     (only when their k-th position blocks).
//!   - Pick the earliest t whose remaining candidate is non-empty; choose the
//!     angle closest to the direct line.

use crate::{Fleet, GameState, Planet, BOARD_SIZE, CENTER_X, CENTER_Y, SUN_RADIUS};
...
/// Sorted, non-overlapping intervals on [0, TAU). Wraps are split.
...
impl AngleSet {
    ...
    pub fn full_arc(center: f64, half: f64) -> Self {
        let mut s = Self::empty();
        s.add_arc(center, half);
        s
    }
...
/// Half-angle subtended at source by a circle of radius `r_target` at distance
/// `d_target`, projected onto the source-circle of radius `d_fleet`. We add a
/// small angular padding so tangent intersections (fleet barely grazing the
/// target) still register — they're real hits in the engine's continuous
/// collision check.
...
/// Find a launch angle and arrival time for `source` -> `target` carrying
/// `num_ships`. `turns_in_future` shifts the entire calculation forward (e.g.,
/// for "if I waited W turns then launched").
///
/// Algorithm (per user spec):
///   1. Build a UNION of valid target-hit arcs across all `t` (the angle
///      ranges where the source-ring at `speed*t` intersects the target).
///   2. Subtract static obstacles (sun + non-orbiting non-comet planets)
///      using the cone formula `asin(r/d_obs)` — one subtraction per obstacle.
///   3. Subtract moving obstacles via an event queue: for each moving
///      obstacle, scan times `k` in [1, max_target_t] and add a "blocking
///      event" only when its distance at k is within (radius + half-step) of
///      the fleet's ring at k. Sort events by k, step through them, and
///      subtract each event's arc from cand.
///   4. Pick the angle closest to the target's current center.
///   5. Compute arrival time by forward-simulating the fleet along that
///      angle until it actually collides with the target.
pub fn dir_to_hit(
    source: &Planet,
    target: &Planet,
    num_ships: i64,
    ...
    // Engine spawns fleets at source_center + (radius+0.1)*dir, so at any
    // turn t the engine fleet is `spawn_offset` units further along the
    // flight line than `speed*t` would imply. Treating the fleet as
    // starting at the center caused us to overshoot orbiting targets by
    // ~1 turn of orbital motion.
    let spawn_offset = source.radius + 0.1;

    // ---- Phase 1: union of target-hit arcs across t ----
    ...
    // ---- Phase 2: subtract static obstacles (cone formula) ----
    let d_sun = dist(src_pos, (CENTER_X, CENTER_Y));
    if d_sun <= SUN_RADIUS {
        return None; // source inside sun shouldn't happen
    }
    ...
    // ---- Phase 3: walk the expanding source-cone outward one step at a
    // time. At each k, for each moving obstacle, subtract its blocking arc
    // from cand if the obstacle could collide with the fleet at that k
    // (its distance from source ≈ fleet's ring distance speed*k).
    ...
    // ---- Phase 4: pick angle closest to target's current center ----
    ...
    // ---- Phase 5: forward-sim along chosen angle to compute arrival time ----
    // Start from the engine's actual spawn position (just outside the source
    // planet along the launch direction), not the planet's center.
...
/// `(planet_id, time)` or `None` if the fleet dies (sun/oob/timeout). `time`
/// follows the same convention as `dir_to_hit`.
