...
decisions live in the Rust extension (`simbot_simplified_native`).
...
The native module (`simbot_simplified_native`) must be compiled into the active
environment before `main.py` can import it:

```powershell
cd bots/mine/simbot-simplified
...
```powershell
python run_match.py simbot-simplified random
```
...
kaggle competitions submit orbit-wars -f bots/mine/simbot-simplified/submission.tar.gz -m "simbot-simplified v1"
```
