...
└── train/
    ...
    ├── build_from_zip.py      — replay zip → summary_v2 NPZ builder
    ├── from_replays_fast.py   — fast Rust extract_v2 replay extraction
    ├── extras_*               — optional extra feature builders
    ├── train_gbm.py           — XGBoost/LightGBM baseline runs
    ├── xgb_tune.py            — XGBoost tuning sweep
    ├── bench_combined.py      — feature-set/model comparison
    ├── topn_experiments.py    — top-N filtered XGBoost experiments
    ├── model_dashboard.py     — HTML metrics/feature dashboard
    ├── eval.py                — fixed-seed eval harness
    ├── check_parity.py        — Python ↔ Rust parity test
    └── weights/               — exported AOWV files
```
