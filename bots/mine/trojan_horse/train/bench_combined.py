...
def train_xgb(X, y, val_mask, n_est=800, depth=6, lr=0.08, return_model=False):
    ...
    result = (((pred > 0.5) == (yb_va > 0.5)).mean(), bst.best_iteration)
    if return_model:
        return result + (bst, pred)
    return result


def dashboard_json_path(base: str | None, tag: str):
    if not base:
        return None
    p = Path(base)
    if p.suffix:
...
def bench(name, X, y, val_mask, do_ensemble=True, dashboard=False, dashboard_top=18, dashboard_sample=20000, dashboard_json=None, dashboard_html=None, permutation=True):
    ...
    t0 = time.time()
    if dashboard:
        s, it, bst, pred = train_xgb(X, y, val_mask, return_model=True)
    else:
        s, it = train_xgb(X, y, val_mask)
    ...
    if dashboard:
        ...
        render_xgb_dashboard(
            ...
            permutation_rows=dashboard_sample,
            json_out=dashboard_json_path(dashboard_json, f"{tag}_xgb_d6"),
            html_out=dashboard_html,
        )
    t0 = time.time()
    if dashboard:
        s, it, bst, pred = train_xgb(X, y, val_mask, depth=8, lr=0.05, n_est=1500, return_model=True)
    else:
        s, it = train_xgb(X, y, val_mask, depth=8, lr=0.05, n_est=1500)
    ...
    if dashboard:
        ...
        render_xgb_dashboard(
            ...
            permutation=permutation,
            permutation_rows=dashboard_sample,
            json_out=dashboard_json_path(dashboard_json, f"{tag}_xgb_d8"),
            html_out=dashboard_html,
        )
...
def main():
    ...
    p.add_argument("--dashboard", action="store_true", help="print compact XGBoost metrics + feature diagnostics")
    p.add_argument("--dashboard-top", type=int, default=18)
    p.add_argument("--dashboard-sample", type=int, default=20000)
    p.add_argument("--dashboard-json", default=None, help="write dashboard JSON; feature/model tag is appended")
    ...
    bench(
        ...
        dashboard_top=args.dashboard_top,
        dashboard_sample=args.dashboard_sample,
