...
def run_xgb(X, y, val_mask, n_est=600, depth=6, lr=0.08, subsample=0.85, colsample=0.85, return_model=False):
    ...
    result = (sign, time.time() - t0, bst.best_iteration)
    if return_model:
        return result + (bst, pred)
    return result


def dashboard_json_path(base: str | None, tag: str) -> Path | None:
    if not base:
        return None
    p = Path(base)
    if p.suffix:
...
def main():
    ...
    p.add_argument("--dashboard-sample", type=int, default=20000, help="rows used for permutation/solo feature probes")
    p.add_argument("--dashboard-json", default=None, help="write dashboard JSON; model tag is appended")
    ...
    p.add_argument("--no-permutation", action="store_true", help="skip permutation importance probes")
    args = p.parse_args()
    ...
    if args.dashboard:
        s, sec, it, bst, pred = run_xgb(X, y, val_mask, return_model=True)
    else:
    ...
    if args.dashboard:
        render_xgb_dashboard(
            ...
            json_out=dashboard_json_path(args.dashboard_json, "xgb_d6_lr008"),
            html_out=dashboard_html,
        )

    print("\n=== XGBoost (depth=8, lr=0.05, n_est=1000) ===")
    if args.dashboard:
        s, sec, it, bst, pred = run_xgb(X, y, val_mask, n_est=1000, depth=8, lr=0.05, return_model=True)
    else:
    ...
    if args.dashboard:
        render_xgb_dashboard(
            ...
            permutation=not args.no_permutation,
            permutation_rows=args.dashboard_sample,
            json_out=dashboard_json_path(args.dashboard_json, "xgb_d8_lr005"),
            html_out=dashboard_html,
        )
