//! Native Orbit Wars agent for the `rust_bot`.
//!
//! This reproduces the open-source `nearest-sniper` baseline exactly: for each
//! planet we own, find the closest planet we don't own and send `garrison + 1`
//! ships if we can afford the takeover, otherwise accumulate.
//!
...
#[pymodule]
fn rust_bot_native(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
