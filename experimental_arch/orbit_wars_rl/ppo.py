...
from .features import ACTION_DIM, MAX_PLANETS, NUM_SEND_OPTIONS, decode_action_index, decode_move, encode_obs
from .model import OrbitPolicy, build_policy
from .visualization import append_jsonl
...
class PPOConfig:
    ...
    device: str = "cpu"
    checkpoint_dir: str = "experimental_arch/checkpoints"
    ...
    # Wandb config. When `use_wandb` is False the trainer logs only to stdout
    # and jsonl files (useful for smoke tests / no-network runs).
    use_wandb: bool = True
    wandb_project: str = "orbit-wars-rl-experimental"
...
def _component_entropies(logits: torch.Tensor) -> dict[str, float]:
    ...
    noop_prob = probs[:, :1]
    pair_probs = probs[:, 1:].reshape(logits.shape[0], MAX_PLANETS, MAX_PLANETS, NUM_SEND_OPTIONS)
...
def train(config: PPOConfig) -> Path:
    ...
    # Wandb setup. If the wandb package isn't installed or use_wandb is False,
    # we silently skip — everything else still works via stdout + jsonl files.
    wandb_run = None
    ...
    while global_step < target_global_step:
        ...
        for _ in range(config.rollout_steps):
            # obs carries `_resolved` from OrbitWarsDuelEnv's per-step cache,
            # so encode_obs uses ground-truth resolution without us threading
            # the env handle through.
            encoded = encode_obs(obs)
        ...
        if wandb_run is not None:
            wandb_run.log({f"train/{k}": v for k, v in metrics.items() if k != "current_opponent"},
                          step=global_step)
