...
def _wandb_log(row: dict, step: int) -> None:
...
def train(config: PPOConfig) -> Path:
    ...
    if config.resume_checkpoint:
        resume_ckpt = torch.load(config.resume_checkpoint, map_location=device, weights_only=False)
    ...
    while global_step < target_global_step:
        ...
        if config.eval_every_updates > 0 and update_idx % config.eval_every_updates == 0:
            ...
            for eval_idx, opponent in enumerate(_names_csv(config.eval_opponents)):
                ...
                eval_metrics[f"avg_reward_{opponent}"] = result["avg_reward"]
            ...
            _wandb_log({f"eval/{key}": value for key, value in eval_metrics.items()}, global_step)
