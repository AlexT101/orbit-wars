...
def load_agent(agent_dir: str) -> Optional[Callable]:
    ...
    If the folder contains a Kaggle-style `submission.tar.gz` and no loose
    `main.py`, the tarball is extracted (with mtime caching) and the extract
    dir is used in place of `agent_dir`. Mirrors kaggle-environments
    agent.py:40-65 behavior beyond that.
    """
    agent_dir_path = ensure_extracted(Path(agent_dir))
    agent_dir = str(agent_dir_path)
    main_py = agent_dir_path / "main.py"
