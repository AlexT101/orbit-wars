...
def scan_zoo(zoo_dir: Path) -> list[AgentInfo]:
    ...
    An agent folder counts if it contains either `main.py` (loose source) or
    `submission.tar.gz` (Kaggle-style packaged submission). Buckets outside
    baselines/external/mine are ignored. Broken agent.yaml falls back to
    folder name, sets last_error.
    """
    ...
    for bucket_dir in sorted(zoo_dir.iterdir()):
        ...
        for agent_dir in sorted(bucket_dir.iterdir()):
            if not agent_dir.is_dir():
                continue
            has_main = (agent_dir / "main.py").is_file()
            has_tarball = (agent_dir / "submission.tar.gz").is_file()
            if not (has_main or has_tarball):
                continue
