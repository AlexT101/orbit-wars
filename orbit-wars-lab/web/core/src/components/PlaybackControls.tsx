...
export const PlaybackControls: React.FC<PlaybackControlsProps> = ({
  ...
  const controlsStyles: React.CSSProperties = {
    ...
    gap: '8px',
    padding: '8px 16px',
    backgroundColor: isDark ? '#1a1a1a' : '#f4f3ef',
    borderRadius: '8px',
    // Narrow-viewport safety: let rows wrap and let flex children shrink
    // below their intrinsic size so step counter / speed select don't
    // push the slider off screen.
    flexWrap: 'wrap',
    minWidth: 0,
  };
  ...
  return (
    <div className={className} style={{ ...controlsStyles, ...style }}>
      ...
      <div style={{ flex: '1 1 120px', position: 'relative', margin: '0 16px', minWidth: '80px' }}>
        <input
          type="range"
          min="0"
          max={maxStep}
